...
export async function DownloadPage({ locale }: { locale: Locale }) {
  ...
  return <ContentPage locale={locale} eyebrow="DOWNLOAD / DIRECT" title={fr ? "Pressay pour macOS." : "Pressay for macOS."} intro={fr ? "Édition Direct universelle, signée Developer ID et notarialisée par Apple." : "Universal Direct edition, Developer ID signed and notarized by Apple."}>
    ...
    <section className="steps"><h2>{fr ? "Installation" : "Installation"}</h2><ol>{(fr ? ["Ouvre le DMG et glisse Pressay dans Applications.", "Lance Pressay et choisis WhisperKit local ou ta clé OpenAI.", "Accorde Microphone, puis Accessibilité au premier usage de l’insertion.", "Maintiens Fn, parle, puis relâche."] : ["Open the DMG and drag Pressay to Applications.", "Launch Pressay and choose local WhisperKit or your OpenAI key.", "Grant Microphone, then Accessibility the first time insertion needs it.", "Hold Fn, speak, then release."]).map((step) => <li key={step}>{step}</li>)}</ol></section>
  </ContentPage>;
}
