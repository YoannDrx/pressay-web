...
export async function DownloadPage({ locale }: { locale: Locale }) {
  ...
  return <ContentPage locale={locale} eyebrow="DOWNLOAD / DIRECT" title={fr ? "Pressay pour macOS." : "Pressay for macOS."} intro={fr ? "Édition Direct Apple Silicon, signée Developer ID et notarialisée par Apple." : "Apple Silicon Direct edition, Developer ID signed and notarized by Apple."}>
    <section className="download-card"><div><span className="mono-label">{release.prerelease ? "PUBLIC BETA" : "PUBLIC STABLE"}</span><h2>{release.tag}</h2><p>macOS 14+ · Apple Silicon · arm64</p></div><div className="download-actions"><Link className="button button-primary" href="/download/pressay">{fr ? "Télécharger Pressay.dmg" : "Download Pressay.dmg"}</Link><a className="inline-arrow" href={release.checksumURL}>{fr ? "Vérifier le SHA-256" : "Verify SHA-256"} →</a></div></section>
