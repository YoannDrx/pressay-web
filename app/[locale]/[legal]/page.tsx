...
function documentFor(locale: Locale, page: LegalPage): { title: string; intro: string; summary: string; sections: Section[] } {
  ...
  const commonIdentity = fr
    ? "Pressay est édité et développé par Yodev, nom commercial de Yoann Andrieux, entrepreneur individuel, 11 rue de la Chine, 75020 Paris, France."
    : "Pressay is published and developed by Yodev, the trading name of Yoann Andrieux, sole trader, 11 rue de la Chine, 75020 Paris, France.";
  const documents: Record<LegalPage, { title: string; intro: string; summary: string; sections: Section[] }> = {
    ...
    terms: fr ? {
      title: "Conditions générales d’utilisation et de vente", intro: "Les règles applicables à Pressay Free, Pro BYOK, Lifetime BYOK et Founding.", summary: "Vous gardez le contrôle de vos données locales ; les offres payantes donnent les droits décrits au moment de l’achat.", sections: [
        ...
        { title: "Offres", bullets: ["Free : usage local/BYOK sans compte obligatoire, dans les limites affichées ;", "Pro BYOK : fonctions avancées, jusqu’à trois Mac, clé fournisseur à votre charge ;", "Lifetime BYOK : accès permanent au périmètre local/BYOK vendu lors de l’achat, hors cloud géré, synchronisation hébergée et Teams ;", "Founding : droit hérité réservé aux installations éligibles et réclamé une seule fois."] },
        ...
        { title: "Prix, essai et renouvellement", paragraphs: ["Les prix finaux sont affichés avant commande. Pro peut inclure un essai de 14 jours sans carte pour les comptes éligibles. L’abonnement mensuel ou annuel se renouvelle jusqu’à annulation. En cas d’échec, les droits peuvent passer en past due puis revenir à Free sans supprimer vos données locales."] },
      ...
      title: "Terms of use and sale", intro: "Rules for Pressay Free, Pro BYOK, Lifetime BYOK and Founding.", summary: "You retain control of local data; paid plans provide the rights described at purchase.", sections: [
        ...
        { title: "Plans", bullets: ["Free: local/BYOK use without a mandatory account, within displayed limits;", "Pro BYOK: advanced features, up to three Macs, provider key at your cost;", "Lifetime BYOK: permanent access to the local/BYOK scope sold at purchase, excluding managed cloud, hosted sync and Teams;", "Founding: inherited entitlement for eligible installations, claimable once."] },
        ...
        { title: "Price, trial and renewal", paragraphs: ["Final prices are shown before purchase. Pro may include a 14-day no-card trial for eligible accounts. Monthly or annual subscriptions renew until cancelled. Payment failure may move access to past due and then Free without deleting local data."] },
    ...
    support: fr ? {
      title: "Support Pressay", intro: "Installer, diagnostiquer et obtenir de l’aide sans exposer le contenu de vos dictées.", summary: "Envoyez la version, macOS, l’app cible, les étapes et le request ID — jamais votre clé ni une dictée.", sections: [
        { title: "Installation et permissions", bullets: ["macOS 14 ou ultérieur, Apple Silicon ou Intel ;", "autoriser le Microphone ;", "pour l’édition Direct, autoriser Accessibilité afin de vérifier la cible et insérer ;", "si une permission reste bloquée, la retirer puis relancer Pressay avant de la redemander."] },
      ...
      title: "Pressay support", intro: "Install, troubleshoot and get help without exposing dictation content.", summary: "Send version, macOS, target app, steps and request ID — never your key or dictation.", sections: [
        { title: "Installation and permissions", bullets: ["macOS 14+, Apple Silicon or Intel;", "allow Microphone;", "for Direct, allow Accessibility to verify targets and insert;", "if a permission is stuck, remove it, relaunch Pressay and request it again."] },
