  ...
  return <ContentPage locale={lang} eyebrow={`LEGAL / ${legal.toUpperCase()}`} title={page.title} intro={page.intro}>
    <div className="legal-layout">
      ...
      <article className="legal-document">
        ...
        <footer className="legal-updated">{lang === "fr" ? "Dernière mise à jour : 10 septembre 2026." : "Last updated: September 10, 2026."}</footer>
      </article>
    </div>
  </ContentPage>;
}
...
function PrivacyTable({ locale }: { locale: Locale }) {
  const rows = locale === "fr" ? [
    ["Compte", "Fournir le compte et sécuriser l’accès", "Contrat", "Better Auth auto-hébergé, Apple, Vercel, Neon", "Durée du compte + délais légaux"],
    ["Appareils", "Appliquer les limites et révoquer un Mac", "Contrat / sécurité", "Neon", "Durée du compte, puis suppression"],
    ["Paiements", "Abonnement, facture, remboursement", "Contrat / obligation légale", "Apple ou Stripe, Vercel, Neon", "Durées comptables et fiscales applicables"],
    ...
    ["Account", "Provide and secure the account", "Contract", "Self-hosted Better Auth, Apple, Vercel, Neon", "Account lifetime plus legal periods"],
    ["Devices", "Apply limits and revoke a Mac", "Contract / security", "Neon", "Account lifetime, then deletion"],
    ["Payments", "Subscription, invoices and refunds", "Contract / legal duty", "Apple or Stripe, Vercel, Neon", "Applicable accounting and tax periods"],
...
function SupportLinks({ locale }: { locale: Locale }) { return <div className="legal-actions"><a className="button button-primary" href="mailto:yoann.andrieux@gmail.com?subject=%5BPressay%5D%20Support">yoann.andrieux@gmail.com</a><Link className="button" href={`/${locale}/download`}>{locale === "fr" ? "Réinstaller Pressay" : "Reinstall Pressay"}</Link></div>; }
function WithdrawalActions({ locale }: { locale: Locale }) { return <div className="legal-actions"><a className="button button-primary" href="mailto:yoann.andrieux@gmail.com?subject=%5BPressay%5D%20R%C3%A9tractation">{locale === "fr" ? "Exercer mon droit" : "Exercise my right"}</a><Link className="button" href="/account">{locale === "fr" ? "Supprimer mon compte" : "Delete my account"}</Link></div>; }
  ...
  const documents: Record<LegalPage, { title: string; intro: string; summary: string; sections: Section[] }> = {
    legal: fr ? {
      title: "Mentions légales", intro: "L’identité de l’éditeur, de l’hébergeur et les informations commerciales de Pressay.", summary: "Pressay est un logiciel macOS édité en France par YoDev.", sections: [
        { title: "Éditeur", paragraphs: [commonIdentity], bullets: ["SIREN : 803 272 590", "SIRET : 803 272 590 00024", "Activité : programmation informatique — NAF 6201Z", "Contact : yoann.andrieux@gmail.com", "Les taxes applicables sont indiquées avant toute commande."] },
        { title: "Direction de la publication", paragraphs: ["Le directeur de la publication est Yoann Andrieux. Les demandes juridiques, de confidentialité et de support peuvent être adressées à yoann.andrieux@gmail.com."] },
        { title: "Hébergement et prestataires", paragraphs: ["Le site, l’API et Better Auth sont hébergés sur Vercel. Les données de compte sont hébergées par Neon. Apple intervient lorsque vous choisissez Se connecter avec Apple. Apple traite les achats effectués dans le Mac App Store ; Stripe traite les achats directs lorsqu’ils sont disponibles. Le contenu vocal n’est pas hébergé par ces prestataires dans les modes local/BYOK de Pressay."] },
        ...
        { title: "Signalement", paragraphs: ["Pour signaler un contenu illicite, une vulnérabilité ou une atteinte à des droits, écrivez à yoann.andrieux@gmail.com en donnant l’URL, les faits et un moyen de vous recontacter. N’envoyez jamais de clé API ni de dictée sensible."] }
      ]
    } : {
      title: "Legal notice", intro: "Publisher, hosting and commercial information for Pressay.", summary: "Pressay is a macOS application published in France by YoDev.", sections: [
        { title: "Publisher", paragraphs: [commonIdentity], bullets: ["SIREN: 803 272 590", "SIRET: 803 272 590 00024", "Business activity: software development — NAF 6201Z", "Contact: yoann.andrieux@gmail.com", "Applicable taxes are shown before any purchase."] },
        { title: "Publication director", paragraphs: ["The publication director is Yoann Andrieux. Legal, privacy and support requests may be sent to yoann.andrieux@gmail.com."] },
        { title: "Hosting and providers", paragraphs: ["The website, API and Better Auth are hosted by Vercel. Account data is hosted by Neon. Apple is involved when you choose Sign in with Apple. Apple processes Mac App Store purchases; Stripe processes direct purchases when available. Voice content is not hosted by these providers in Pressay local/BYOK modes."] },
        ...
        { title: "Reporting", paragraphs: ["To report unlawful content, a vulnerability or a rights issue, email yoann.andrieux@gmail.com with the URL, facts and a contact method. Never send an API key or sensitive dictation."] }
      ]
    },
    privacy: fr ? {
      title: "Politique de confidentialité", intro: "Ce que Pressay traite, pourquoi, pendant combien de temps — et surtout ce qui reste sur votre Mac.", summary: "Pressay monitore les métadonnées commerciales et techniques nécessaires, jamais le contenu de vos dictées.", sections: [
        { title: "Responsable du traitement", paragraphs: [commonIdentity, "YoDev est responsable des traitements liés au compte, aux paiements, appareils, codes, referrals, téléchargements, support et audit. Contact RGPD : yoann.andrieux@gmail.com."] },
      ...
      title: "Privacy policy", intro: "What Pressay processes, why, for how long — and what remains on your Mac.", summary: "Pressay monitors necessary commercial and technical metadata, never dictation content.", sections: [
        { title: "Controller", paragraphs: [commonIdentity, "YoDev controls processing for accounts, payments, devices, codes, referrals, downloads, support and audit. Privacy contact: yoann.andrieux@gmail.com."] },
        ...
        { title: "Your rights", paragraphs: ["You may request access, correction, deletion, restriction, objection and portability where applicable. Consent may be withdrawn without retroactive effect. Email yoann.andrieux@gmail.com; proportionate identity evidence may be requested."], bullets: ["Response normally within one month;", "complaint available to the French CNIL or your local authority;", "account deletion from the account area;", "separate retention only for records required by law."] },
    ...
    terms: fr ? {
      ...
      title: "Terms of use and sale", intro: "Rules for Pressay Free and Pressay Pro.", summary: "You retain control of local data; the Pro subscription provides the rights described at purchase.", sections: [
        ...
        { title: "Statutory digital conformity", paragraphs: ["Consumers benefit from the statutory conformity guarantee applicable to digital content and services, independently of any commercial warranty. Remedies, periods and conditions provided by applicable consumer law remain available. Contact yoann.andrieux@gmail.com with the order reference and a description of the issue, without attaching dictation or an API key."] },
        { title: "Cancellation, withdrawal and refunds", paragraphs: ["For a Mac App Store purchase, manage or cancel your subscription in your Apple account settings and request a refund at reportaproblem.apple.com. For a direct purchase, cancel through the Stripe portal; access normally continues to the paid period end. No extra commercial refund promise is made beyond applicable law. Consumer withdrawal and consent to immediate performance are explained on the dedicated page."] },
        ...
        { title: "Law and disputes", paragraphs: ["French law applies without removing mandatory protections in a consumer’s country. Contact yoann.andrieux@gmail.com first. Consumer mediator details must be completed and validated before commercial launch; paid launch remains blocked until then."] }
      ]
    },
    cookies: fr ? {
      title: "Politique de cookies", intro: "Une liste courte et lisible des cookies nécessaires au fonctionnement du site.", summary: "Pressay n’utilise aucun cookie publicitaire ni fingerprint marketing.", sections: [
        ...
        { title: "Contact", paragraphs: ["Pour toute question ou pour signaler un écart entre cette liste et le comportement observé : yoann.andrieux@gmail.com."] }
      ]
    } : {
      title: "Cookie policy", intro: "A short, readable list of cookies required by the site.", summary: "Pressay uses no advertising cookie or marketing fingerprint.", sections: [
        ...
        { title: "Contact", paragraphs: ["Questions or reports of a mismatch between this list and observed behaviour: yoann.andrieux@gmail.com."] }
      ]
    },
    support: fr ? {
      title: "Support Pressay", intro: "Installer, diagnostiquer et obtenir de l’aide sans exposer le contenu de vos dictées.", summary: "Envoyez la version, macOS, l’app cible, les étapes et le request ID — jamais votre clé ni une dictée.", sections: [
        ...
        { title: "Facturation, codes et parrainage", paragraphs: ["Les achats Mac App Store sont gérés par Apple ; utilisez « Restaurer les achats » dans Pressay pour retrouver vos droits avec le compte Apple utilisé lors de l’achat. Pour les achats directs, le portail Stripe gère moyen de paiement, factures, changement mensuel/annuel et annulation. Un code gratuit complète le droit effectif sans écraser Stripe. Une récompense de parrainage apparaît après le premier paiement éligible, jamais après une facture à zéro."] },
      ...
      title: "Pressay support", intro: "Install, troubleshoot and get help without exposing dictation content.", summary: "Send version, macOS, target app, steps and request ID — never your key or dictation.", sections: [
        ...
        { title: "Billing, codes and referrals", paragraphs: ["Mac App Store purchases are managed by Apple; use Restore Purchases in Pressay with the Apple account used for purchase. For direct purchases, Stripe’s portal manages payment method, invoices, monthly/annual switch and cancellation. A free code complements effective access without overwriting Stripe. Referral rewards appear after the first eligible paid invoice, never a zero invoice."] },
    ...
    withdrawal: fr ? {
      title: "Rétractation et suppression de compte", intro: "Exercer vos droits de consommateur et comprendre les effets de la suppression.", summary: "Le droit légal s’applique sans garantie commerciale supplémentaire.", sections: [
        ...
        { title: "Comment exercer", paragraphs: ["Envoyez avant l’échéance une déclaration dénuée d’ambiguïté à yoann.andrieux@gmail.com avec l’email du compte, la date et la référence de commande. N’envoyez aucune donnée bancaire. Un accusé de réception sera conservé."] },
      ...
      title: "Withdrawal and account deletion", intro: "Exercise consumer rights and understand deletion effects.", summary: "Statutory rights apply without an extra commercial refund promise.", sections: [
        ...
        { title: "How to exercise", paragraphs: ["Before the deadline, email an unambiguous statement to yoann.andrieux@gmail.com with account email, date and order reference. Never send bank data. An acknowledgement will be retained."] },
