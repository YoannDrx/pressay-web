...
export async function POST(request: Request) {
  if (!commercialCheckoutIsEnabled())
    return NextResponse.json(
      { error: "commercial_launch_not_enabled" },
      { status: 503 },
    );
  ...
  const referralCode = verifyReferralCookie(
    referralCookie,
    process.env.PRESSAY_REFERRAL_COOKIE_SECRET,
  );
  if (referralCode) {
    const attribution = await pressayAPI(
      "referrals/attribute",
      {
        method: "POST",
    ...
    if (attribution.status === 401 || attribution.status >= 500) {
      ...
      const failure = await attribution
        .clone()
        .json()
        .catch(() => null);
      // Sales and referrals have separate launch switches. A paused referral
      // program must not prevent an otherwise authorized purchase.
      if (failure?.error?.code !== "referrals_disabled")
